...
class pluginClass:
    ...
    def initGui(self):
      ...
      self.action = QAction(QIcon(icon), 'My Plugin', self.iface.mainWindow())    # create QAction
      self.action.triggered.connect(self.run)                           # if QAction triggered, run
      self.iface.addPluginToMenu('&My Plugin>', self.action)       # add action to menu
    ...
    def unload(self):
      ...
      self.iface.removePluginMenu('&Save Attributes', self.action)      # remove action from plugin menu
      del self.action
...
#      print(QgsExpression.checkExpression(vlgstring))
